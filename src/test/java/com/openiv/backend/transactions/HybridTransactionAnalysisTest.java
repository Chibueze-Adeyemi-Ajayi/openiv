 ...
 *
 * Strategy:
 * - Timestamp anomalies: tested via an inline helper that mirrors production
 * logic exactly.
 * - Geo-velocity: tested via GeoVelocityChecker (public static API).
 * - Institution thresholds / KYC tiers / behavioral patterns / customer
 * immediate rules:
 * tested via Java reflection on the private scoring methods.
 * - Customer aggregate rules: tested via inline helpers that mirror production
 * logic.
 ...
 *
 * After all tests a Markdown report is written to
 * target/aml-rule-coverage-report.md.
 */
...
public class HybridTransactionAnalysisTest {
    ...
    static BehavioralRuleRecord bRule(String ruleId, boolean active,
        ...
        return new BehavioralRuleRecord(
                ...
                active, 0, "recent", params, new JsonArray(), null, null, null, null);
    }
    ...
    // ───────────────────────────────────────────────────────────────────────────
    // Scoring formula helper — mirrors final score computation in
    // analyzeTransaction()
    // ───────────────────────────────────────────────────────────────────────────
    ...
    static Stream<Arguments> impossibleTravelNormal() {
        ...
        return Stream.of(
                ...
                Arguments.of("Same location (GPS noise)", locTxn(6.5, 3.4, base),
                        Optional.of(locTxn(6.5, 3.401, base.minusMinutes(5)))),
                Arguments.of("10km in 10 min (60 km/h)", locTxn(6.5, 3.4, base),
                        Optional.of(locTxn(6.6, 3.4, base.minusMinutes(10)))),
                Arguments.of("50km in 30 min (100 km/h)", locTxn(6.5, 3.4, base),
                        Optional.of(locTxn(6.9, 3.4, base.minusMinutes(30)))),
                Arguments.of("100km in 1h (100 km/h)", locTxn(6.5, 3.4, base),
                        Optional.of(locTxn(7.4, 3.4, base.minusHours(1)))),
                Arguments.of("200km in 1h (200 km/h)", locTxn(6.5, 3.4, base),
                        Optional.of(locTxn(8.3, 3.4, base.minusHours(1)))),
                Arguments.of("Gap > 24h (stale skip)", locTxn(6.5, 3.4, base),
                        Optional.of(locTxn(0.0, 0.0, base.minusDays(2)))),
                Arguments.of("Missing lat on current",
                        txn(BigDecimal.valueOf(100), "wire", "outward", base, null, null, "FB", "GT", "t", "c"),
                        Optional.of(locTxn(6.5, 3.4, base.minusMinutes(5)))),
                Arguments.of("Missing lat on previous", locTxn(6.5, 3.4, base),
                        Optional.of(txn(BigDecimal.valueOf(100), "wire", "outward", base.minusMinutes(5), null, null,
                                "FB", "GT", "t", "c"))),
                Arguments.of("Distance < 1km (noise)", locTxn(6.5000, 3.4000, base),
                        Optional.of(locTxn(6.5004, 3.4001, base.minusMinutes(1)))));
    }
    ...
    static Stream<Arguments> impossibleTravelAbnormal() {
        ...
        return Stream.of(
                Arguments.of("Lagos→London in 1h (~5200 km/h)", locTxn(51.5, -0.12, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(1)))),
                Arguments.of("Lagos→NYC in 2h (~4300 km/h)", locTxn(40.7, -74.0, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(2)))),
                Arguments.of("100km in 3s (near-simultaneous)", locTxn(7.4, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusSeconds(3)))),
                Arguments.of("500km in 10 min (~3000 km/h)", locTxn(10.5, 7.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusMinutes(10)))),
                Arguments.of("1500km in 1h (1500 km/h)", locTxn(20.0, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(1)))),
                Arguments.of("2000km in 1h (2000 km/h)", locTxn(24.5, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(1)))),
                Arguments.of("60km in 1s (simultaneous > 50km)", locTxn(7.04, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusSeconds(1)))),
                Arguments.of("3000km in 2h (1500 km/h)", locTxn(33.0, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(2)))),
                Arguments.of("Intercontinental in 30min", locTxn(48.8, 2.35, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusMinutes(30)))),
                Arguments.of("Cross-equator in 1h (>1050 km/h)", locTxn(-3.0, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(1)))));
    }
    ...
    static Stream<Arguments> suspiciousTravelNormal() {
        ...
        return Stream.of(
                Arguments.of("50 km/h — ground travel", locTxn(6.95, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(1)))), // ~50km
                Arguments.of("100 km/h — highway", locTxn(7.4, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(1)))), // ~100km
                Arguments.of("200 km/h — fast ground", locTxn(8.3, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(1)))), // ~200km
                Arguments.of("400 km/h — helicopter", locTxn(10.1, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(1)))), // ~400km
                Arguments.of("700 km/h — slow jet", locTxn(12.8, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(1)))), // ~700km
                Arguments.of("800 km/h — just under 900", locTxn(13.7, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(1)))), // ~800km
                Arguments.of("893 km/h — clearly under 900", locTxn(14.55, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(1)))), // ~890km
                Arguments.of("No previous txn", locTxn(51.5, -0.12, base), Optional.empty()),
                Arguments.of("No coordinates",
                        txn(BigDecimal.TEN, "wire", "outward", base, null, null, "FB", "GT", "t", "c"),
                        Optional.empty()),
                Arguments.of("Gap > 24h skipped", locTxn(51.5, -0.12, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusDays(2)))));
    }
    ...
    static Stream<Arguments> suspiciousTravelAbnormal() {
        ...
        return Stream.of(
                Arguments.of("905 km in 1h → 905 km/h", locTxn(14.65, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(1)))),
                Arguments.of("920 km in 1h → 920 km/h", locTxn(14.8, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(1)))),
                Arguments.of("950 km in 1h → 950 km/h", locTxn(15.1, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(1)))),
                Arguments.of("1000 km in 1h → 1000 km/h", locTxn(15.5, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(1)))),
                Arguments.of("1020 km in 1h → 1020 km/h", locTxn(15.72, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(1)))),
                Arguments.of("930 km in 1h", locTxn(14.9, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(1)))),
                Arguments.of("910 km/h — 2h window 1820km", locTxn(22.8, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(2)))),
                Arguments.of("940 km/h implied", locTxn(15.0, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(1)))),
                Arguments.of("960 km/h implied", locTxn(15.2, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(1)))),
                Arguments.of("980 km/h implied", locTxn(15.38, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(1)))));
    }
    ...
    static Stream<Arguments> airTravelNormal() {
        ...
        return Stream.of(
                ...
                Arguments.of("493 km/h (under 500)", locTxn(10.94, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(1)))),
                Arguments.of("No previous", locTxn(10.0, 3.4, base), Optional.empty()),
                Arguments.of("900 km/h → SUSPICIOUS not AIR", locTxn(14.65, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(1)))),
                Arguments.of("GPS noise < 1km", locTxn(6.5004, 3.4001, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusMinutes(1)))),
                Arguments.of("> 24h gap stale", locTxn(10.0, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusDays(2)))),
                Arguments.of("Missing coordinates",
                        txn(BigDecimal.TEN, "wire", "outward", base, null, null, "FB", "GT", "t", "c"),
                        Optional.empty()));
    }
    ...
    static Stream<Arguments> airTravelAbnormal() {
        ...
        return Stream.of(
                ...
                Arguments.of("2h: 1100km → 550 km/h", locTxn(16.4, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(2)))));
    }
    ...
    static Stream<Arguments> highVelocityNormal() {
        OffsetDateTime base = OffsetDateTime.now(WAT);
        return Stream.of(
                Arguments.of("Walking 5 km/h", locTxn(6.54, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(1)))),
                Arguments.of("50 km/h — city drive", locTxn(6.95, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(1)))),
                Arguments.of("100 km/h — highway", locTxn(7.4, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(1)))),
                Arguments.of("150 km/h", locTxn(7.85, 3.4, base), Optional.of(locTxn(6.5, 3.4, base.minusHours(1)))),
                Arguments.of("199 km/h — just under", locTxn(8.29, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(1)))),
                Arguments.of("500 km/h → AIR not HIGH", locTxn(11.0, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(1)))),
                Arguments.of("No previous", locTxn(8.0, 3.4, base), Optional.empty()),
                Arguments.of("> 24h gap stale", locTxn(8.0, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusDays(2)))),
                Arguments.of("GPS noise", locTxn(6.5001, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusMinutes(5)))),
                Arguments.of("196 km/h (well under 200)", locTxn(8.27, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(1)))));
    }
    ...
    static Stream<Arguments> highVelocityAbnormal() {
        ...
        return Stream.of(
                ...
                Arguments.of("2h: 500km → 250 km/h", locTxn(11.0, 3.4, base),
                        Optional.of(locTxn(6.5, 3.4, base.minusHours(2)))),
    ...
    static final ThresholdRecord WIRE_RULE = threshold("high-value-wire", true, 5_000_000L, 5_000_000L, 35);
    ...
    static final ThresholdRecord VEL_RULE = threshold("velocity-cluster", true, 10L, 10L, 25);
    ...
    void velocityClusterNormal(String desc, long count24h, String dir) throws Exception {
        ...
        var rule = desc.contains("inactive")
                ? threshold("velocity-cluster", false, 10L, 10L, 25)
                : VEL_RULE;
    ...
    static final ThresholdRecord LATE_RULE = threshold("late-night-large", true, 1_000_000L, 1_000_000L, 30);
    ...
    void lateNightNormal(String desc, OffsetDateTime at, BigDecimal amount) throws Exception {
        var rule = desc.contains("inactive")
                ? threshold("late-night-large", false, 1_000_000L, 1_000_000L, 30)
                : LATE_RULE;
    ...
    static final ThresholdRecord VS_RULE = threshold("velocity-spike", true, 130L, 130L, 20);
    ...
    void velocitySpikeNormal(String desc, long todayCnt, int expected) throws Exception {
        ...
        var rule = desc.contains("inactive")
                ? threshold("velocity-spike", false, 130L, 130L, 20)
                : VS_RULE;
    ...
    static final ThresholdRecord BDC_RULE = threshold("cross-border-bdc", true, 2_000_000L, 2_000_000L, 30);
    ...
    void bdcNormal(String desc, BigDecimal amount, String channel, String dir) throws Exception {
        var t = simpleTxn(amount, channel, dir, OffsetDateTime.now(WAT));
        var rule = desc.contains("inactive")
                ? threshold("cross-border-bdc", false, 2_000_000L, 2_000_000L, 30)
                : BDC_RULE;
        var r = scoreThresholds(t, List.of(rule), List.of(), 0, 0, 0, false, false, 0,
                BigDecimal.ZERO, 1000, Optional.empty());
    ...
    static final ThresholdRecord DORMANT_RULE = threshold("dormant-reactivation", true, 500_000L, 500_000L, 25);
    ...
    static Stream<Arguments> dormantNormal() {
        ...
        return Stream.of(
                ...
                Arguments.of("90 days (boundary, not over)", Optional.of(now.minusDays(90)),
                        BigDecimal.valueOf(5_000_000)),
                Arguments.of("91 days but amount under 500k", Optional.of(now.minusDays(91)),
                        BigDecimal.valueOf(499_999)),
                Arguments.of("100 days, amount = threshold", Optional.of(now.minusDays(100)),
                        BigDecimal.valueOf(500_000)),
                Arguments.of("rule inactive, 365 days 5M", Optional.of(now.minusDays(365)),
                        BigDecimal.valueOf(5_000_000)),
    ...
    void dormantNormal(String desc, Optional<OffsetDateTime> lastTxn, BigDecimal amount) throws Exception {
        ...
        var rule = desc.contains("inactive")
                ? threshold("dormant-reactivation", false, 500_000L, 500_000L, 25)
                : DORMANT_RULE;
        var r = scoreThresholds(t, List.of(rule), List.of(), 0, 0, 0, false, false, 0,
    ...
    static KycTierRecord kycTier0() {
        // tier=0, single limit per channel = 200,000 NGN, daily = 1,000,000 NGN, no
        // boost
    ...
    static Stream<Arguments> behavDevNormal() {
        ...
        return Stream.of(
                Arguments.of("normal amount within 3σ", BigDecimal.valueOf(120_000), "wire", "FirstBank", midday,
                        "transfer", stableProfile()),
                Arguments.of("avg amount", BigDecimal.valueOf(100_000), "wire", "FirstBank", midday, "transfer",
                        stableProfile()),
                Arguments.of("avg + 2.9σ = 158k", BigDecimal.valueOf(158_000), "wire", "FirstBank", midday, "transfer",
                        stableProfile()),
                Arguments.of("typical channel (wire)", BigDecimal.valueOf(100_000), "wire", "FirstBank", midday,
                        "transfer", stableProfile()),
                Arguments.of("within hour window (14:00)", BigDecimal.valueOf(100_000), "wire", "FirstBank",
                        OffsetDateTime.now(WAT).withHour(14), "transfer", stableProfile()),
                Arguments.of("known recipient bank", BigDecimal.valueOf(100_000), "wire", "FirstBank", midday,
                        "transfer", stableProfile()),
                Arguments.of("known category", BigDecimal.valueOf(100_000), "wire", "FirstBank", midday, "transfer",
                        stableProfile()),
                Arguments.of("profile count < 10 (ignored)", BigDecimal.valueOf(999_999), "bdc", "UnknownBank",
                        midday.withHour(3), "crypto",
                        profile(100_000, 20_000, List.of("wire"), 8, 18, List.of("FirstBank"), List.of("transfer"), 9)),
                Arguments.of("no profile (null)", BigDecimal.valueOf(999_999), "bdc", "UnknownBank", midday, "crypto",
                        null),
                Arguments.of("stddev = 0 (skip amount check)", BigDecimal.valueOf(999_999), "wire", "FirstBank", midday,
                        "transfer",
    ...
    static Stream<Arguments> behavDevAbnormal() {
        ...
        return Stream.of(
                Arguments.of("amount > avg + 3σ = 160001", BigDecimal.valueOf(160_001), "wire", "FirstBank", midday,
                        "transfer", stableProfile()),
                Arguments.of("unknown channel (mobile)", BigDecimal.valueOf(100_000), "mobile", "FirstBank", midday,
                        "transfer", stableProfile()),
                Arguments.of("outside hour window (03:00)", BigDecimal.valueOf(100_000), "wire", "FirstBank", lateNt,
                        "transfer", stableProfile()),
                Arguments.of("unknown bank (GTBank)", BigDecimal.valueOf(100_000), "wire", "GTBank", midday, "transfer",
                        stableProfile()),
                Arguments.of("unknown category (crypto)", BigDecimal.valueOf(100_000), "wire", "FirstBank", midday,
                        "crypto", stableProfile()),
                Arguments.of("all deviations together", BigDecimal.valueOf(999_999), "mobile", "GTBank", lateNt,
                        "crypto", stableProfile()),
                Arguments.of("200k outward (1σ above 3σ)", BigDecimal.valueOf(200_000), "wire", "FirstBank", midday,
                        "transfer", stableProfile()),
                Arguments.of("amount 5σ above avg", BigDecimal.valueOf(200_001), "wire", "FirstBank", midday,
                        "transfer", stableProfile()),
                Arguments.of("new bank UBA", BigDecimal.valueOf(100_000), "wire", "UBA", midday, "transfer",
                        stableProfile()),
                Arguments.of("new channel + out-of-hours", BigDecimal.valueOf(100_000), "ussd", "FirstBank", lateNt,
                        "transfer", stableProfile()));
    }
    ...
    void blockedBankNormal(String desc, String recipientBank, String senderBank, List<String> blocked)
            throws Exception {
        var arr = new JsonArray();
        blocked.forEach(arr::add);
    ...
    void blockedBankAbnormal(String desc, String recipientBank, String senderBank, List<String> blocked)
            throws Exception {
        var arr = new JsonArray();
        blocked.forEach(arr::add);
        var rule = cRule("blocked_banks", "flag", "both", new JsonObject().put("banks", arr));
    ...
    static Stream<Arguments> allowedBankAbnormal() {
        return Stream.of(
                ...
                Arguments.of("exact but different casing fail", "ZenithBank", List.of("first")) // "first" not in
                                                                                                // "zenithbank"
        );
    }
    ...
    static Stream<Arguments> dailyLimitNormal() {
        return Stream.of(
                ...
                Arguments.of("today=500k, txn=500k, limit=1M", BigDecimal.valueOf(500_000), BigDecimal.valueOf(500_000),
                        1_000_000L),
                Arguments.of("today+txn exactly = limit", BigDecimal.valueOf(500_000), BigDecimal.valueOf(500_000),
                        1_000_000L),
                ...
                Arguments.of("huge limit, small amount", BigDecimal.valueOf(999_999), BigDecimal.valueOf(999_999),
                        Long.MAX_VALUE),
    ...
    static Stream<Arguments> dailyLimitAbnormal() {
        return Stream.of(
                ...
                Arguments.of("today=500k, txn=501k, limit=1M", BigDecimal.valueOf(500_000), BigDecimal.valueOf(501_000),
                        1_000_000L),
                Arguments.of("today=999k, txn=2k, limit=1M", BigDecimal.valueOf(999_000), BigDecimal.valueOf(2_000),
                        1_000_000L),
                ...
                Arguments.of("today=500k+1, txn=500k, limit=1M", BigDecimal.valueOf(500_001),
                        BigDecimal.valueOf(500_000), 1_000_000L),
                Arguments.of("today=900k, txn=200k, limit=1M", BigDecimal.valueOf(900_000), BigDecimal.valueOf(200_000),
                        1_000_000L),
                Arguments.of("today=1M-1, txn=2, limit=1M", BigDecimal.valueOf(999_999), BigDecimal.valueOf(2),
                        1_000_000L));
    }
    ...
    static Stream<Arguments> monthlyLimitNormal() {
        return Stream.of(
                ...
                Arguments.of("2.5M + 2.5M = 5M (boundary)", BigDecimal.valueOf(2_500_000),
                        BigDecimal.valueOf(2_500_000), 5_000_000L),
                Arguments.of("0 + 0 = 0, limit=0", BigDecimal.ZERO, BigDecimal.ZERO, 1L),
                Arguments.of("4.9M + 99k = 4.999M, limit=5M", BigDecimal.valueOf(4_900_000), BigDecimal.valueOf(99_000),
                        5_000_000L),
                ...
                Arguments.of("month=9.9M, txn=99k, limit=10M", BigDecimal.valueOf(9_900_000),
                        BigDecimal.valueOf(99_000), 10_000_000L),
                Arguments.of("exact boundary 10M", BigDecimal.valueOf(9_000_000), BigDecimal.valueOf(1_000_000),
                        10_000_000L),
                Arguments.of("month=1M, limit=MAX_VALUE", BigDecimal.valueOf(1_000_000),
                        BigDecimal.valueOf(999_999_999L), Long.MAX_VALUE));
    }
    ...
    static Stream<Arguments> monthlyLimitAbnormal() {
        return Stream.of(
                ...
                Arguments.of("2.5M + 2.6M, limit=5M", BigDecimal.valueOf(2_500_000), BigDecimal.valueOf(2_600_000),
                        5_000_000L),
                Arguments.of("4.9M + 200k, limit=5M", BigDecimal.valueOf(4_900_000), BigDecimal.valueOf(200_000),
                        5_000_000L),
                Arguments.of("5M + 1, limit=5M", BigDecimal.valueOf(5_000_000), BigDecimal.ONE, 5_000_000L),
                Arguments.of("9M + 2M, limit=10M", BigDecimal.valueOf(9_000_000), BigDecimal.valueOf(2_000_000),
                        10_000_000L),
                ...
                Arguments.of("4.999M + 2, limit=5M", BigDecimal.valueOf(4_999_000), BigDecimal.valueOf(2_000),
                        5_000_000L),
    ...
    static Stream<Arguments> rapidWdNormal() {
        return Stream.of(
                Arguments.of("inward direction (not outward)", false, BigDecimal.valueOf(1_000_000),
                        BigDecimal.valueOf(500_000), 0.5),
                Arguments.of("no recent deposit", true, BigDecimal.ZERO, BigDecimal.valueOf(500_000), 0.5),
                Arguments.of("outward, ratio 0.49 under 0.5 threshold", true, BigDecimal.valueOf(1_000_000),
                        BigDecimal.valueOf(490_000), 0.5),
                Arguments.of("ratio 0.49 under threshold", true, BigDecimal.valueOf(1_000_000),
                        BigDecimal.valueOf(490_000), 0.5),
                Arguments.of("small deposit, small withdrawal (ratio OK)", true, BigDecimal.valueOf(1_000),
                        BigDecimal.valueOf(499), 0.5),
                Arguments.of("outward, ratio 0.3 under 0.5", true, BigDecimal.valueOf(1_000_000),
                        BigDecimal.valueOf(300_000), 0.5),
                Arguments.of("deposit=0, outward", true, BigDecimal.ZERO, BigDecimal.valueOf(100), 0.5),
                Arguments.of("ratio 0.4 under 0.8 threshold", true, BigDecimal.valueOf(1_000_000),
                        BigDecimal.valueOf(400_000), 0.8),
                Arguments.of("ratio 0.79 under 0.8", true, BigDecimal.valueOf(1_000_000), BigDecimal.valueOf(790_000),
                        0.8),
                Arguments.of("inward + deposit + ratio over", false, BigDecimal.valueOf(1_000_000),
                        BigDecimal.valueOf(600_000), 0.5));
    }

    static Stream<Arguments> rapidWdAbnormal() {
        return Stream.of(
                Arguments.of("ratio 0.5 = threshold (>=)", true, BigDecimal.valueOf(1_000_000),
                        BigDecimal.valueOf(500_000), 0.5),
                Arguments.of("ratio 0.51 just over 0.5", true, BigDecimal.valueOf(1_000_000),
                        BigDecimal.valueOf(510_000), 0.5),
                Arguments.of("ratio 1.0 (full withdrawal)", true, BigDecimal.valueOf(1_000_000),
                        BigDecimal.valueOf(1_000_000), 0.5),
                Arguments.of("ratio 1.5 (over deposit)", true, BigDecimal.valueOf(1_000_000),
                        BigDecimal.valueOf(1_500_000), 0.5),
                Arguments.of("ratio 0.8 >= 0.5 threshold", true, BigDecimal.valueOf(500_000),
                        BigDecimal.valueOf(400_000), 0.5),
                Arguments.of("ratio 0.9 threshold 0.8", true, BigDecimal.valueOf(1_000_000),
                        BigDecimal.valueOf(900_000), 0.8),
                Arguments.of("ratio 2.0 (double deposit)", true, BigDecimal.valueOf(200_000),
                        BigDecimal.valueOf(400_000), 0.5),
                Arguments.of("very high deposit, 50% out", true, BigDecimal.valueOf(10_000_000),
                        BigDecimal.valueOf(5_000_001), 0.5),
                Arguments.of("ratio 0.6 >= 0.5", true, BigDecimal.valueOf(1_000_000), BigDecimal.valueOf(600_000), 0.5),
                Arguments.of("threshold 0.3, ratio 0.4", true, BigDecimal.valueOf(1_000_000),
                        BigDecimal.valueOf(400_000), 0.3));
    }
    ...
    @Test
    @DisplayName("Score: no rules → 0")
    void scoreNoRules() {
        assertThat(computeScore(List.of(), 0)).isEqualTo(0);
    }

    @Test
    @DisplayName("Score: single rule 35 → 35")
    void scoreSingleRule() {
        assertThat(computeScore(List.of(35), 0)).isEqualTo(35);
    }

    @Test
    @DisplayName("Score: two rules 35+25 → 35+7=42")
    void scoreTwoRules() {
        assertThat(computeScore(List.of(35, 25), 0)).isEqualTo(42);
    }

    @Test
    @DisplayName("Score: three rules 35+25+25 → 35+14=49")
    void scoreThreeRules() {
        assertThat(computeScore(List.of(35, 25, 25), 0)).isEqualTo(49);
    }

    @Test
    @DisplayName("Score: customer risk ≥70 → +15 premium")
    void scoreHighRiskPremium() {
        assertThat(computeScore(List.of(35), 70)).isEqualTo(50);
    }

    @Test
    @DisplayName("Score: customer risk ≥40 < 70 → +8 premium")
    void scoreMedRiskPremium() {
        assertThat(computeScore(List.of(35), 50)).isEqualTo(43);
    }

    @Test
    @DisplayName("Score: customer risk <40 → no premium")
    void scoreLowRiskPremium() {
        assertThat(computeScore(List.of(35), 39)).isEqualTo(35);
    }

    @Test
    @DisplayName("Score: capped at 100 regardless of violations")
    void scoreCapAt100() {
        assertThat(computeScore(List.of(93, 70, 55, 50), 15)).isEqualTo(100);
    }

    @Test
    @DisplayName("Score: 4 rules, maxScore=55, (4-1)×7=21, score=76 > 75 → critical")
    ...
    @Test
    @DisplayName("Score: 60≤score<75 → priority = high")
    void scorePriorityHigh() {
        assertThat(TransactionScorer.getPriority(65)).isEqualTo("high");
    }

    @Test
    @DisplayName("Score: 40≤score<60 → priority = medium")
    void scorePriorityMedium() {
        assertThat(TransactionScorer.getPriority(50)).isEqualTo("medium");
    }

    @Test
    @DisplayName("Score: <40 → priority = low")
    void scorePriorityLow() {
        assertThat(TransactionScorer.getPriority(20)).isEqualTo("low");
    }
    ...
    static void generateReport() throws Exception {
        ...
        if (!anyFail)
            sb.append("_None — all tests passed._\n");

        sb.append("\n## Rule Descriptions\n\n");
        sb.append("| Rule ID | Category | Description |\n|---|---|---|\n");
        List.of(
                new String[] { "MICRO_TIMING_ANOMALY", "Timestamp",
                        "occurred_at within ±5s of server time (score 25)" },
                new String[] { "STALE_TIMESTAMP_ANOMALY", "Timestamp",
                        "occurred_at more than 24h in the past (score 55)" },
                new String[] { "FUTURE_TIMESTAMP_ANOMALY", "Timestamp",
                        "occurred_at more than beam+1h in the future (score 70)" },
                new String[] { "TXN_IMPOSSIBLE_TRAVEL", "Geo-velocity",
                        ">1050 km/h or <30s across >50km (hard score 93, auto-case)" },
                new String[] { "TXN_SUSPICIOUS_TRAVEL", "Geo-velocity",
                        "900–1050 km/h — outer limit of commercial aviation (score 60)" },
                new String[] { "TXN_AIR_TRAVEL_REQUIRED", "Geo-velocity",
                        "500–900 km/h — requires air travel (score 35)" },
                new String[] { "TXN_HIGH_VELOCITY", "Geo-velocity",
                        "200–500 km/h — very fast ground/helicopter (score 15)" },
                new String[] { "high-value-wire", "Institution",
                        "wire transfer amount > configured threshold (default score 35)" },
                new String[] { "velocity-cluster", "Institution",
                        "customer 24h txn count > configured count threshold (score 25)" },
                new String[] { "late-night-large", "Institution", "large transfer between 22:00–05:59 (score 30)" },
                new String[] { "OTP_ALERT", "Institution",
                        "OTP attack correlation — failed auth before txn (score 50)" },
                new String[] { "VELOCITY_SPIKE", "Institution", "today txn count > expected × spike ratio (score 20)" },
                new String[] { "cross-border-bdc", "Institution",
                        "BDC channel transaction above threshold (score 30)" },
                new String[] { "DORMANT_REACTIVATION", "Institution",
                        "account inactive >90 days + large transaction (score 25)" },
                new String[] { "KYC_TIER_LIMIT_EXCEEDED", "KYC",
                        "single txn exceeds per-channel limit for customer's KYC tier (score 45+boost)" },
                new String[] { "KYC_TIER_DAILY_LIMIT_EXCEEDED", "KYC",
                        "cumulative daily spend exceeds tier daily cap (score 50+boost)" },
                new String[] { "pat-4", "Behavioral",
                        "institution today > yesterday × spike_ratio (score depends on severity)" },
                ...
                new String[] { "BEHAVIORAL_PATTERN_DEVIATION", "Behavioral",
                        "txn deviates from customer profile in amount/channel/time/bank/category" },
                new String[] { "CUSTOMER_RULE_MAX_AMOUNT", "Customer", "single txn amount > customer-specific max" },
                new String[] { "CUSTOMER_RULE_BLOCKED_BANK", "Customer",
                        "recipient/sender bank is in customer's blocklist" },
                new String[] { "CUSTOMER_RULE_BANK_NOT_ALLOWED", "Customer",
                        "destination bank not in customer's whitelist" },
                ...
                new String[] { "CUSTOMER_RULE_MONTHLY_LIMIT", "Customer",
                        "this month's sum + txn > customer monthly cap" },
                new String[] { "CUSTOMER_RULE_VELOCITY", "Customer", "customer txn count in window >= max_count" },
                new String[] { "RAPID_POST_DEPOSIT_WITHDRAWAL", "Customer",
                        "outward amount / recent deposit >= min_withdrawal_ratio" },
                new String[] { "SUDDEN_WITHDRAWAL_AFTER_DEPOSIT", "Customer",
                        "outward txn within N minutes of deposit + amount >= min_amount" })
                .forEach(row -> sb.append("| ").append(row[0]).append(" | ").append(row[1]).append(" | ").append(row[2])
                        .append(" |\n"));
