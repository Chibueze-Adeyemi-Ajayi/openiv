 ...
 */
public final class DojahClient {

  private static final Logger log = LoggerFactory.getLogger(DojahClient.class);

  private final WebClient httpClient;
  private final DojahConfig config;

  public DojahClient(WebClient httpClient, DojahConfig config) {
    this.httpClient = httpClient;
    this.config = config;
  }

  public DojahConfig config() {
    return config;
  }

  // ── BVN Full Lookup ───────────────────────────────────────────────────────

  public Future<DojahVerificationResult> verifyBvn(String bvn) {
    if (!config.isConfigured())
      return Future.succeededFuture(DojahVerificationResult.unverified("bvn", bvn, "not_configured"));
    ...
    return httpClient
        ...
        .compose(resp -> {
          ...
          if (resp.statusCode() != 200) {
            return Future.succeededFuture(DojahVerificationResult.unverified("bvn", bvn, raw));
          }
        ...
        .recover(err -> {
          ...
          return Future.succeededFuture(DojahVerificationResult.unverified("bvn", bvn, err.getMessage()));
        });
  }

  // ── NIN Lookup ────────────────────────────────────────────────────────────

  public Future<DojahVerificationResult> verifyNin(String nin) {
    if (!config.isConfigured())
      return Future.succeededFuture(DojahVerificationResult.unverified("nin", nin, "not_configured"));
    ...
    return httpClient
        ...
        .compose(resp -> {
          ...
          if (resp.statusCode() != 200) {
            return Future.succeededFuture(DojahVerificationResult.unverified("nin", nin, raw));
          }
        ...
        .recover(err -> {
          ...
          return Future.succeededFuture(DojahVerificationResult.unverified("nin", nin, err.getMessage()));
        });
  }

  // ── Phone Number Lookup ───────────────────────────────────────────────────

  public Future<DojahVerificationResult> lookupPhone(String phone) {
    if (!config.isConfigured())
      return Future.succeededFuture(DojahVerificationResult.unverified("phone", phone, "not_configured"));
    ...
    return httpClient
        ...
        .compose(resp -> {
          ...
          if (resp.statusCode() == 404) {
            return Future.succeededFuture(DojahVerificationResult.notFound("phone", phone));
          }
          if (resp.statusCode() != 200) {
            return Future.succeededFuture(DojahVerificationResult.unverified("phone", phone, raw));
          }
        ...
        .recover(err -> {
          ...
          return Future.succeededFuture(DojahVerificationResult.unverified("phone", phone, err.getMessage()));
        });
  }

  // ── BVN + Selfie Verify ───────────────────────────────────────────────────

  public Future<DojahVerificationResult> verifyBvnWithSelfie(String bvn, String selfieBase64) {
    if (!config.isConfigured())
      return Future.succeededFuture(DojahVerificationResult.unverified("bvn", bvn, "not_configured"));
  ...
  public Future<DojahVerificationResult> verifyNinWithSelfie(String nin, String selfieBase64) {
    if (!config.isConfigured())
      return Future.succeededFuture(DojahVerificationResult.unverified("nin", nin, "not_configured"));
  ...
  public Future<PhoneFraudResult> screenPhoneFraud(String phone) {
    ...
    return httpClient
        ...
        .<PhoneFraudResult>map(resp -> {
          ...
          if (resp.statusCode() == 404)
            return PhoneFraudResult.notFound(phone);
          if (resp.statusCode() != 200)
            return PhoneFraudResult.unresolved(raw);
  ...
  public Future<NubanResult> lookupNuban(String accountNumber, String bankCode) {
    ...
    return httpClient
        ...
        .<NubanResult>map(resp -> {
          ...
          if (resp.statusCode() != 200)
            return NubanResult.unresolved(raw);
  ...
  /**
   * Submit front (and optionally back) of an ID document for analysis.
   * Endpoint: POST /api/v1/document/analysis
   *
   * @param imageFront base64-encoded front image (data-URI prefix stripped)
   * @param imageBack  base64-encoded back image, or null/blank to omit
   * @return JsonObject with entity.status.overall_status (1=valid, 0=invalid)
   */
  public Future<JsonObject> verifyDocument(String imageFront, String imageBack) {
    if (!config.isConfigured())
      return Future.succeededFuture(new JsonObject().put("error", "not_configured"));

    String front = stripDataUri(imageFront);
    JsonObject body = new JsonObject()
        .put("input_type", "base64")
        .put("imagefrontside", front);
    if (imageBack != null && !imageBack.isBlank()) {
      body.put("imagebackside", stripDataUri(imageBack));
    }

    log.info("[Doja] >> POST {}/api/v1/document/analysis", config.baseUrl());

    return httpClient
        .postAbs(config.baseUrl() + "/api/v1/document/analysis")
        .putHeader("AppId", config.appId())
        .putHeader("Authorization", config.apiKey())
        ...
        .map(resp -> {
          String raw = resp.bodyAsString();
          log.info("[Doja] << POST document/analysis | status={} body_len={}", resp.statusCode(), raw.length());
          if (resp.statusCode() != 200)
            return new JsonObject().put("error", raw);
          return extractSseJson(raw);
        })
        .recover(err -> {
          log.error("[Doja] !! POST document/analysis error: {}", err.getMessage());
  ...
  private DojahVerificationResult parseBvnLookup(String bvn, JsonObject json) {
    ...
    return new DojahVerificationResult(
        ...
        false, -1,
        json.encode(),
        cleanBase64(e.getString("image")));
  }

  private DojahVerificationResult parseNinLookup(String nin, JsonObject json) {
    JsonObject e = json.getJsonObject("entity", json);
    return new DojahVerificationResult(
        "nin", nin, hasName(e),
        e.getString("first_name"),
        ...
        false, -1,
        json.encode(),
        cleanBase64(e.getString("photo")));
  }

  private DojahVerificationResult parsePhoneLookup(String phone, JsonObject json) {
    JsonObject e = json.getJsonObject("entity", json);
    String firstName = e.getString("firstName");
    String lastName  = e.getString("lastname");  // Dojah uses lowercase 'n'
  ...
  private NubanResult parseNubanLookup(JsonObject json, String rawJson) {
    ...
    if (firstName == null || firstName.isBlank())
      return NubanResult.unresolved(rawJson);
  ...
  private DojahVerificationResult parseSelfieVerify(String type, String ref, JsonObject json) {
    JsonObject e = json.getJsonObject("entity", json);
  ...
  static JsonObject extractSseJson(String body) {
    ...
    if (trimmed.startsWith("{")) {
      try {
        ...
        // Dojah may embed raw newlines inside JSON string values (e.g. photo base64)
        try {
          return new JsonObject(trimmed.replaceAll("[\\r\\n]", ""));
    ...
    for (int i = 0; i < lines.length; i++) {
      if (lines[i].startsWith("data: ")) {
        String candidate = lines[i].substring(6).trim();
        if (!candidate.isEmpty() && !candidate.equals("[DONE]")) {
          lastData = candidate;
          lastDataPos = trimmed.lastIndexOf("data: ");
        }
      }
    }
    if (lastData != null) {
      try {
        ...
        // lastData is a broken JSON fragment — Dojah embedded raw newlines in an SSE data line.
        // Re-extract from the raw body starting at the last data: offset, strip embedded newlines.
        if (lastDataPos >= 0) {
          String raw = trimmed.substring(lastDataPos + 6).replaceAll("[\\r\\n]", "");
          try {
            return new JsonObject(raw);
          } catch (Exception e2) {
            log.warn("[Doja] Failed to parse SSE JSON even after newline strip: {}", e2.getMessage());
          }
        }
  ...
  /**
   * Strip data-URI prefix (data:image/jpeg;base64,…) so the raw base64 remains.
   */
  private static String stripDataUri(String b64) {
    if (b64 == null)
      return "";
    return b64.contains(",") ? b64.substring(b64.indexOf(',') + 1).trim() : b64.trim();
  }
  ...
  private static byte[] decodeImage(String b64) {
    ...
    try {
      return Base64.getDecoder().decode(stripped.trim());
  ...
  /** Strip MIME line-breaks from base64 so browsers accept it in data URIs. */
  private static String cleanBase64(String b64) {
    if (b64 == null) return null;
    String clean = b64.replaceAll("[\\r\\n\\s]", "");
